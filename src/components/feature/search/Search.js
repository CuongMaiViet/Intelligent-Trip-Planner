import React, { useContext, useState } from 'react'
import { Globalstate } from '../../../GlobalState'
import './Search.scss'

export default function Search({ page }) {
    const state = useContext(Globalstate)
    // const [auth] = state.userInfoAPI.auth
    const [authAdmin] = state.userInfoAPI.authAdmin
    //Place
    const [searchP, setSearchP] = state.haNoiPlaceAPI.search
    const [categoryP, setCategoryP] = state.haNoiPlaceAPI.category
    //Accommodation
    const [searchA, setSearchA] = state.accommodationAPI.search
    const [categoryA, setCategoryA] = state.accommodationAPI.category
    //Cuisine
    const [searchC, setSearchC] = state.cuisinesAPI.search
    const [categoryC, setCategoryC] = state.cuisinesAPI.category

    const [disable, setDisable] = useState(true)

    const clearSearch = () => {
        setSearchA('')
        setSearchP('')
        setSearchC('')
    }

    const clearFilter = () => {
        setCategoryA('')
        setCategoryP('')
        setCategoryC('')
        document.querySelector('input[type="radio"]:checked').checked = false
        setDisable(true)
        console.log(categoryA);
        console.log(categoryP);
        console.log(categoryC);
    }

    return (
        <div className='features'>
            {!authAdmin && (
                <div className="filter flex">
                    {/* <span>Filter:</span> */}
                    {page === 'accommodation' && (
                        <>
                            <label className='flex' htmlFor="is3stars">
                                <div className="star">
                                    <i className="fas fa-star"></i>
                                    <i className="fas fa-star"></i>
                                    <i className="fas fa-star"></i>
                                </div>
                                <input type="radio" name='category' id="is3stars"
                                    value='is3stars=1' onChange={e => {
                                        setCategoryA(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>
                            <label className='flex' htmlFor="is4stars">
                                <div className="star">
                                    <i className="fas fa-star"></i>
                                    <i className="fas fa-star"></i>
                                    <i className="fas fa-star"></i>
                                    <i className="fas fa-star"></i>
                                </div>
                                <input type="radio" name='category' id="is4stars"
                                    value='is4stars=1' onChange={e => {
                                        setCategoryA(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>
                            <label className='flex' htmlFor="is5stars">
                                <div className="star">
                                    <i className="fas fa-star"></i>
                                    <i className="fas fa-star"></i>
                                    <i className="fas fa-star"></i>
                                    <i className="fas fa-star"></i>
                                    <i className="fas fa-star"></i>
                                </div>
                                <input type="radio" name='category' id="is5stars"
                                    value='is5stars=1' onChange={e => {
                                        setCategoryA(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>
                            <label className='flex' htmlFor="isHomestay">
                                <div className="star">
                                    <i className="fas fa-home" ></i>
                                </div>
                                <input type="radio" name='category' id="isHomestay"
                                    value='isHomestay=1' onChange={e => {
                                        setCategoryA(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>

                        </>
                    )}
                    {page === 'place' && (
                        <div className='grid places'>
                            <label className='landmark flex' htmlFor="isHistorical">
                                <div className="lankmark">
                                    <i className="fas fa-synagogue"></i> Historical
                                </div>
                                <input type="radio" name='category' id="isHistorical"
                                    value='isHistorical=1' onChange={e => {
                                        setCategoryP(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>

                            <label className='landmark flex' htmlFor="isAdventure">
                                <div className="lankmark">
                                    <i className="fas fa-hiking"></i> Adventure
                                </div>
                                <input type="radio" name='category' id="isAdventure"
                                    value='isAdventure=1' onChange={e => {
                                        setCategoryP(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>

                            <label className='landmark flex' htmlFor="isMuseum">
                                <div className="lankmark">
                                    <i className="fas fa-landmark"></i> Museum
                                </div>
                                <input type="radio" name='category' id="isMuseum"
                                    value='isMuseum=1' onChange={e => {
                                        setCategoryP(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>

                            <label className='landmark flex' htmlFor="isNature">
                                <div className="lankmark">
                                    <i className="fas fa-tree"></i> Nature
                                </div>
                                <input type="radio" name='category' id="isNature"
                                    value='isNature=1' onChange={e => {
                                        setCategoryP(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>

                            <label className='landmark flex' htmlFor="isShopping">
                                <div className="lankmark">
                                    <i className="fas fa-store"></i> Shopping
                                </div>
                                <input type="radio" name='category' id="isShopping"
                                    value='isShopping=1' onChange={e => {
                                        setCategoryP(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>

                            <label className='landmark flex' htmlFor="isReligious">
                                <div className="lankmark">
                                    <i className="fas fa-church"></i> Religious
                                </div>
                                <input type="radio" name='category' id="isReligious"
                                    value='isReligious=1' onChange={e => {
                                        setCategoryP(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>

                            <label className='landmark flex' htmlFor="isUrban">
                                <div className="lankmark">
                                    <i className="fas fa-city"></i> Urban
                                </div>
                                <input type="radio" name='category' id="isUrban"
                                    value='isUrban=1' onChange={e => {
                                        setCategoryP(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>

                            <label className='landmark flex' htmlFor="isPark">
                                <div className="lankmark">
                                    <i className="fas fa-street-view"></i> Park
                                </div>
                                <input type="radio" name='category' id="isPark"
                                    value='isPark=1' onChange={e => {
                                        setCategoryP(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>
                        </div>
                    )}
                    {page === 'cuisine' && (
                        <div className='grid cuisines'>
                            <label className='cuisine flex' htmlFor="isVietnamese">
                                <div className="cuisine flex">
                                    <img alt="alter" src="data:image/svg+xml;base64," />
                                    Vietnamese
                                </div>
                                <input type="radio" name='category' id="isVietnamese"
                                    value='isVietnamese=1' onChange={e => {
                                        setCategoryC(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>

                            <label className='cuisine flex' htmlFor="isWestern">
                                <div className="cuisine flex">
                                    <img alt="alter" src="data:image/svg+xml;base64," />
                                    Western
                                </div>
                                <input type="radio" name='category' id="isWestern"
                                    value='isWestern=1' onChange={e => {
                                        setCategoryC(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>

                            <label className='cuisine flex' htmlFor="isThai">
                                <div className="cuisine flex">
                                    <img alt="alter" src="data:image/svg+xml;base64," />
                                    Thailand
                                </div>
                                <input type="radio" name='category' id="isThai"
                                    value='isThai=1' onChange={e => {
                                        setCategoryC(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>

                            <label className='cuisine flex' htmlFor="isKorean">
                                <div className="cuisine flex">
                                    <img alt="alter" src="data:image/svg+xml;base64," />
                                    Korean
                                </div>
                                <input type="radio" name='category' id="isKorean"
                                    value='isKorean=1' onChange={e => {
                                        setCategoryC(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>

                            <label className='cuisine flex' htmlFor="isJapanese">
                                <div className="cuisine flex">
                                    <img alt="alter" src="data:image/svg+xml;base64," />
                                    Japanese
                                </div>
                                <input type="radio" name='category' id="isJapanese"
                                    value='isJapanese=1' onChange={e => {
                                        setCategoryC(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>

                            <label className='cuisine flex' htmlFor="isIndian">
                                <div className="cuisine flex">
                                    <img alt="alter" src="data:image/svg+xml;base64," />
                                    Indian
                                </div>
                                <input type="radio" name='category' id="isIndian"
                                    value='isIndian=1' onChange={e => {
                                        setCategoryC(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>

                            <label className='cuisine flex' htmlFor="isChinese">
                                <div className="cuisine flex">
                                    <img alt="alter" src="data:image/svg+xml;base64," />
                                    Chinese
                                </div>
                                <input type="radio" name='category' id="isChinese"
                                    value='isChinese=1' onChange={e => {
                                        setCategoryC(e.target.value)
                                        setDisable(false)
                                    }} />
                                <div className="check">
                                    <i className="fas fa-check"></i>
                                </div>
                            </label>
                        </div>
                    )}
                    <button className='erase flex' onClick={clearFilter} disabled={disable}>
                        <div className="star">
                            <i className="fas fa-ban"></i>
                        </div>
                    </button>
                </div>
            )}

            <div className='search-bar'>
                <input className='search' type="text"
                    value={searchA || searchP || searchC}
                    placeholder='Search by name'
                    autoComplete='on'
                    onChange={e => {
                        if (page === 'place') setSearchP(e.target.value)
                        if (page === 'accommodation') setSearchA(e.target.value)
                        if (page === 'cuisine') setSearchC(e.target.value)
                        // setCurrentPage(1)
                    }} />
                <button className="clearSearch" onClick={clearSearch}>
                    <i className="fas fa-times"></i>
                </button>
            </div>
        </div>
    )
}
